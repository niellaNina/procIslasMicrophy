def preselect_original(ds, rm_cirrus = True, rm_c_T = -35, marine = True, m_lat = 70, rm_roll = True, roll_th = 5, rm_head = True, head_th = 1, rm_dist=""):
...
def preselect(ds, rm_cirrus = True, rm_c_T = -35, marine = True, m_lat = 70, rm_roll = True, roll_th = 5, rm_head = True, head_th = 1, rm_dist=""):
    ...
    combined_mask = None  # will become a DataArray of booleans

    # --- marine ---
    if marine:
        pre_text += f"only marine (lat>{m_lat}), "
        marinelat_mask = (ds["lat"] >= m_lat)          # add .compute() here if needed
        combined_mask = marinelat_mask if combined_mask is None else combined_mask & marinelat_mask

    # --- cirrus ---
    if rm_cirrus:
        pre_text += f"Cirrus removed (>{rm_c_T}), "
        cirrusT_mask = (ds["T"] >= rm_c_T)
        combined_mask = cirrusT_mask if combined_mask is None else combined_mask & cirrusT_mask

    # --- roll ---
    if rm_roll:
        pre_text += f"Roll less than {roll_th} removed, "
        roll_mask = (abs(ds["ROLL"]) <= roll_th)
        combined_mask = roll_mask if combined_mask is None else combined_mask & roll_mask

    # --- heading gradient ---
    if rm_head:
        pre_text += f"Heading below {head_th}removed, "
        head_mask = (abs(ds["dfdx_thead"]) <= head_th)
        combined_mask = head_mask if combined_mask is None else combined_mask & head_mask

      # --- distance ---
    if isinstance(rm_dist, int):
        pre_text += f"distance_from_ice < {rm_dist} km, "
        dist_mask = (ds["distance_from_ice"] < rm_dist)
        combined_mask = dist_mask if combined_mask is None else combined_mask & dist_mask

    # If no selection was applied, define mask as True everywhere on a core variable
    if combined_mask is None:
        # pick a representative variable; adjust if needed
        core = ds["lat"]
        combined_mask = xr.full_like(core, True, dtype=bool)

    # If you want everything computed (optional):
    combined_mask = combined_mask.compute()

    # Use the combined mask to select relevant subset
    ds_relevant = ds.where(combined_mask, drop=True)

    # Add the flag to the original ds
    ds = ds.assign(relevant_mcao_flag=combined_mask)
    ds['relevant_mcao_flag'] = ds['relevant_mcao_flag'].assign_attrs(
        long_name = 'Flag relevant mCAO area',
        description = f'Flag that covers selection of data that are within the area that covers mCAO clouds. Selection: {pre_text}',
        unit = 'boolean'
    )

    return ds, ds_relevant, pre_text
...
def incloud_select(ds, lwc, iwc,  th_method='LWC_IWC_th',lwc_th = 0.01, iwc_th = 0.01,n_ice_th = 0.1,n_drp_th = 2, name_suff = ""):
    """ Function to preselect the dataset to use in composite analysis
    ...
    if th_method == 'LWC_th':
        ...
        incloud_text = "LWC > 0.01 g m−3, IWC > 0.01 g m−3"
    ...
    # update ds with flag
    incloud_flag = incloud_mask.astype(bool)
    
    ds = ds.assign(incloud_flag=incloud_flag)
    ds['incloud_flag'] = ds['incloud_flag'].assign_attrs(
            long_name = 'Flag for incloud values',
            description = f'Flag that covers selection of data that are incloud according to the given incloud selection method: {incloud_text}. Based on parameters: {iwc} and {lwc}.',
            unit = 'boolean'
        )

    return ds, ds_incloud, th_method, iwc_th 
