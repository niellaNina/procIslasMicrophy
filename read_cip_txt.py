...
from pathlib import Path #for stripping file extention from filename
...
for flight in flights:
    ...
    nav_file = glob.glob(path_flight + flight + nav_file_struct) # returns a list, must access with file[0]
    ...
    for textfile in txt:
        ...
        start_bin=end_v.pop(0)
        bins_df = pd.DataFrame({mid_h: mid_v, end_h: end_v}) #dataframe with bin information
        ...
        head = var[0].strip() # use first entry as column header
        var_df = pd.DataFrame(var[1:len(var)], columns=[head]) # create dataframe excluding the first entry (which is used as header)
        # clean up and separate variable from unit 
        var_df['unit'] = var_df['Variables:'].apply(lambda x: x.split('[')[1].strip())
        var_df['Variables:'] = var_df['Variables:'].apply(lambda x: x.split('[')[0].strip())
